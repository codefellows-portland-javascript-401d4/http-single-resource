...
const server = http.createServer((req, res) => {
    ...
    } else if (req.url === '/teams/seahawks' && req.method === 'POST') {
        bodyReader(req, (err, data) => {
            if(err) {
                res.statusCode = 400;
                res.end(err.message);
            } else {
                footballStore.writeFile('/teams/seahawks.json', data).then(data => {
                    console.log(data);
                    res.statusCode = 200;
                    res.write('data has written');
                    res.end();
                })
                .catch(function(err) {
                    console.log(err);
    ...
    } else if (req.url === '/teams/seahawks' && req.method === 'PUT') {
        bodyReader(req, (err, data) => {
            if(err) {
                res.statusCode = 400;
                res.end(err.message);
    ...
    } else if (req.url === '/teams/seahawks' && req.method === 'DELETE') {
        footballStore.removeFile('teams/seahawks.json').then(data => {
            console.log(data);
            res.statusCode = 200;
